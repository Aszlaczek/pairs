...
const Buttons = (props: { list: string[], again: number, action: Function, countF: Function, count: number }) => {
    ...
    useEffect(() => {
        ...
        props.list.map((e, i) => {
            let card: Card = { id: i.toString(), attribute: e, done: false, clicked: false }
            ...
            one.attribute === card.attribute && one.id !== card.id
                ...
                : setTimeout(() => {
                    updateItem = updateItem.map(e => {
                        return e.id === card.id || e.id === one.id ? { ...e, clicked: false } : e
                    })
                    setItems(updateItem)
                }, 500)
            setActive(!active)
    ...
    const makeGame = () => {
        return items?.every(e => e.done === true)
            ? <span><h1>Koniec Gry</h1> <button type='button' onClick={() => reset()}>Zagraj ponownie?</button></span>
            : items?.map((e, i) => {
                return (
                    <div key={i} className={`item ${active && one!.id === i.toString() ? 'check' : ''}`}>
                        <Button card={e} action={() => chose(e)} pick={active} first={one} /></div >
                )
            })
    }
    return (
